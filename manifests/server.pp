...
#
# $metrics_reporter_enabled    - Enable or Disable metrics reporter.  Default: false
#
# $metrics_dir                 - Directory in which to store metrics CSVs.  Default: /tmp/kafka_metrics
#
class kafka::server(
    $enabled                         = true,
    ...
    $metrics_reporter_enabled        = $kafka::defaults::metrics_reporter_enabled,
...
) inherits kafka::defaults
{
    ...
    $broker_port = inline_template('<%= scope.lookupvar("kafka::hosts")[scope.lookupvar("::fqdn")].fetch("port", 9092) %>')
    ...
    service { 'kafka':
        ...
        require    => [File['/etc/kafka/server.properties'], File['/etc/default/kafka'], File[$log_dir]],
    }
}
