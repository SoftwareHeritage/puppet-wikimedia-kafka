...
) inherits kafka::defaults
{
    ...
    if $use_systemd {
        file { '/etc/systemd/system/kafka.d':
            ensure => 'directory',
        }
        file { '/etc/systemd/system/kafka.d/override.conf':
        ...
        exec { 'systemd-reload-for-kafka-override':
            ...
            refreshonly => true,
            subscribe   => File['/etc/systemd/system/kafka.d/override.conf'],
        }

        # Make kafka service require this exec, but not subscribe to it
        Service['kafka'] {
            require => Exec['systemd-reload-for-kafka-override'],
        }
    }
